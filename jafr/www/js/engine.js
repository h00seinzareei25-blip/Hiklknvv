...
(function (global) {
  ...
  function natqRulesBlock(meta, opts) {
    ...
    if (isChoice) {
      ...
      lines.push('نطق یک‌خطی: <جملهٔ کامل ۸–۲۵ کلمه‌ای فقط از حروف مخزن؛ سبک کلاسیک جدولی>');
      lines.push('### ۳) تفسیر هوش مصنوعی (اجباری)');
      lines.push('تفسیر: <۲–۵ جمله روان؛ معنای نطق یک‌خطی را برای صورت‌مسئله بگو؛ در نزاع طرفین را نام ببر>');
      lines.push('### پایان');
  ...
  function buildJudgePrompt(meta, contextLines, generatorAnswer) {
    ...
    return [
      ...
      '- نطق یک‌خطی نهایی: <جملهٔ کامل کلاسیک از حروف مخزن>',
    ...
    const generator = [
      ...
      ...(rules.isChoice ? ['1) فقط گزینه‌های سؤال را مقایسه کن.', '2) جدول پوشش + عنصر + مدخل + پایداری را مبنا بگیر.', '3) ۳ تا ۵ رتبه + ۲–۴ جمله توضیح بده.', '4) کاندیدهای پایدار را علامت بزن.']
        ...
        : sentenceNatq ? [
          ...
          '3) جدول کاندید پشتیبان بده (معنادار؛ پنجرهٔ بی‌معنا غالب نشود).',
          '4) «تفسیر هوش مصنوعی» جدا بنویس و در نزاع طرفین را نام ببر.',
          '5) قضاوت نهایی با داور.'
        ]
